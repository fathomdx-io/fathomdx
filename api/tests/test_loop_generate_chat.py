...
async def test_chat_passes_messages_and_returns_assistant_dict():
    ...
    with patch.object(llm, "_resolve_client_and_model", return_value=(fake_client, "model-x", "hard")):
...
async def test_chat_normalizes_none_content_to_empty_string():
    ...
    with patch.object(llm, "_resolve_client_and_model", return_value=(fake_client, "m", "hard")):
